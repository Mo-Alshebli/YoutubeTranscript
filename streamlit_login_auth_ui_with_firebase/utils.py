...
def check_usr_pass_and_get_email(username: str, password: str):
    ref = db.reference('users')
    users = ref.get()
    for uid, user_details in users.items():
        if user_details['username'] == username:
            try:
                password_verified = ph.verify(user_details['password'], password)
                return password_verified, user_details.get('email', None)
            except:
                pass
    return False, None
