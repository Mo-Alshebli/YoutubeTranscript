...
from .utils import  check_usr_pass_and_get_email
...
class __login__:
    ...
    def get_email(self):
        """
        Retrieves the current user's email. This implementation assumes
        the email is stored in a similar way to the username.
        """
        if st.session_state.get('LOGOUT_BUTTON_HIT') == False:
            fetched_cookies = self.cookies
            if '__streamlit_login_signup_ui_email__' in fetched_cookies.keys():
                return fetched_cookies['__streamlit_login_signup_ui_email__']
        return None
        ...
        if st.session_state['LOGGED_IN'] == False:
            ...
            with del_login.form("Login Form"):
                ...
                if login_submit_button == True:
                    authenticate_user_check, user_email = check_usr_pass_and_get_email(username, password)
                    ...
                    if authenticate_user_check == False:
                        ...
                        self.cookies['__streamlit_login_signup_ui_email__'] = user_email  # Store email in cookies
                        self.cookies.save()
                        del_login.empty()
    ...
    def sign_up_widget(self) -> None:
        """
        Creates the sign-up widget and stores the user info in a secure way in the firebase.json file.
        """
    ...
    def reset_password(self) -> None:
        ...
        resets the password and updates the same in the firebase.json file.
        """
